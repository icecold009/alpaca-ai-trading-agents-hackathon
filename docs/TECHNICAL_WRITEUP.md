...
The current release candidate has verified local backend/frontend tests, a reproducible verifier, browser/accessibility smoke coverage, and a zero-finding security review. The official Alpaca MCP/CLI requirement, a fresh-account market-open paper order and terminal lifecycle, hosted deployment, and final submission artifacts remain release gates. No live-money execution path is supported, and no performance claim should be inferred from the recorded fixtures.
